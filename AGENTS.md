...
- 作業量が比較的大きく、実装範囲を他の作業から独立して切り出せる場合は、原則として `gpt-5.6-sol` を主担当、`gpt-5.6-luna` の推論レベル `max` を実装担当のサブエージェントとして活用する。小規模な変更、同じファイルを密接に編集する変更、または分割による調整コストが実装コストを上回る変更は、主担当が直接行う。
- 主担当は、要件整理、仕様・設計、セキュリティ境界の判断、作業分割、統合、最終レビュー、最終検証、ユーザーへの報告を担当する。サブエージェントへ、セキュリティ上の最終判断、仕様矛盾の解消、権限拡大の判断を委ねない。
- サブエージェントへ渡す作業は、単独で完了判定できる単位に限定する。依頼時には、目的、受け入れ条件、変更してよいファイル、変更してはいけないファイル、維持すべき公開インターフェースと不変条件、必要なテスト、時間制限または停止条件を明示する。実現手法は、要件や既存設計が特定の方法を要求する場合を除き、過度に指定しない。
- 主担当とサブエージェントの編集対象を重複させない。並行作業中に対象の重複や予期しない変更を検出した場合は、そのまま上書きせず、作業分担を見直す。
- サブエージェントには、コミット、push、外部サービスへの書き込み、破壊的操作、秘密情報の取得・表示を行わせない。これらが必要になった場合は作業を停止し、主担当へ理由と必要な操作を報告させる。
- 対話入力を待つ処理、長時間継続する処理、GUI を開く検証をサブエージェントへ任せる場合は、可能な限り非対話の検証経路を用意し、明示した時間制限で終了させる。時間制限を超えた場合は待機を続けず、取得済みの結果、未完了項目、停止理由を主担当へ返させる。
- サブエージェントの完了報告には、変更したファイル、実行した検証、検証結果、未検証事項、残存リスクを含める。主担当は報告だけを根拠に完了扱いせず、差分と重要な実装を自ら確認し、必要な検証を独立して再実行してから統合する。
- モデルまたはサブエージェント機能を利用できない場合は、無理に同等の分割を再現せず、主担当が作業を継続し、最終報告で委譲を行わなかった理由を簡潔に示す。
