...
param(
    ...
    [ValidateSet('Install', 'Verify', 'Cleanup')]
    [string]$Mode,
    ...
    [Parameter(Mandatory = $false)]
    [string]$OutputBase = (Join-Path $PSScriptRoot 'WFP-PhaseC-C1-Results')
)
...
if ($Mode -eq 'Install') {
    if (-not [string]::IsNullOrWhiteSpace($StatePath)) {
        throw 'Do not supply -StatePath in Install mode.'
    }
...
if ($Mode -eq 'Cleanup') {
    ...
    $state.cleaned_at = (Get-Date).ToString('o')
