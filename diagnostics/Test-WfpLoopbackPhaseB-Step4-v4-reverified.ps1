...
foreach ($item in @(
    ...
    if ($f.sublayer_found -and
        $f.sublayer_key -ne $ExpectedAppIsolationSubLayerKey) {
        $precheckErrors.Add(
            ("{0}: unexpected App Isolation sublayer key: {1}" -f
                $item.label,
                $f.sublayer_key))
    }

    if ($f.condition_count -ne 1) {
        $precheckErrors.Add(
            ("{0}: expected exactly one AppContainerLoopback condition, actual {1}." -f
                $item.label,
                $f.condition_count))
    } else {
        $referenceConditions = @($f.conditions)

        if ($referenceConditions.Count -ne 1) {
            $precheckErrors.Add(
                ("{0}: condition array count mismatch." -f $item.label))
        } else {
            $referenceCondition = $referenceConditions[0]

            if ($referenceCondition.field_key -ne $ExpectedFlagsConditionKey) {
                $precheckErrors.Add(
                    ("{0}: reference condition is not FWPM_CONDITION_FLAGS." -f
                        $item.label))
            }

            if ($referenceCondition.match_type -ne 6) {
                $precheckErrors.Add(
                    ("{0}: reference condition match type is not FLAGS_ALL_SET." -f
                        $item.label))
            }

            if ($referenceCondition.value_type -ne 3) {
                $precheckErrors.Add(
                    ("{0}: reference condition value type is not FWP_UINT32." -f
                        $item.label))
            }

            if ($referenceCondition.uint32_value -ne 1) {
                $precheckErrors.Add(
                    ("{0}: reference condition loopback flag value is not 1." -f
                        $item.label))
            }
        }
    }
...
$result = [ordered]@{
    ...
    interop_layout = [ordered]@{
        ...
        errors = $layoutErrors.ToArray()
    }

    precheck = [ordered]@{
        passed = $precheckPassed
        errors = $precheckErrors.ToArray()
        expected_app_isolation_sublayer_key = $ExpectedAppIsolationSubLayerKey
        expected_flags_condition_key = $ExpectedFlagsConditionKey
