...
On Windows, Approved Host processes use a new process group and no window. A Codex Sandbox route is launched suspended, assigned to a per-operation Windows Job Object, and resumed only after assignment. The Job enforces active-process and aggregate committed-memory limits and kill-on-close over the complete launcher/command descendant tree. On other platforms processes use a new session. Process groups/sessions alone are lifecycle control, not an OS sandbox.
