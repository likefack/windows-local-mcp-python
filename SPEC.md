...
`git_enabled` also does not make Automatic Git available by itself. Automatic Git requires an explicit Git path/hash identity, the current generic Codex Sandbox live evidence, every Sandbox security property verified for the stricter Automatic Git policy, and an exact Git-specific live marker produced by `verify-git-broker`. The marker is bound to the Git identity, Sandbox/backend evidence, workspace, configured scratch quota, containment-policy generation v7, Automatic Git command-policy generation v5, trusted process-cwd/fixed-`-C` policy, exact projection ownership-trust policy, sanitized `core.autocrlf`／attribute stat semantics, and required-builtin policy. `git_info` / `execute_readonly` remain public surfaces regardless of current availability, so session capability data must keep configured, enabled, available, and Windows-live-verified separate. A missing, failed, or stale Git-specific marker is a fail-closed unavailable state, not a reason to run a weaker Git child.

LocalMCP server startup independently manages the generic Codex Sandbox live-verification lifecycle. It validates the current backend and marker without delaying Broker transport readiness. A current schema-v6 marker whose lifecycle status, backend, WFP/OS/account, full isolation-context digest, and TTL remain valid is reused without a full probe. Missing, stale, incompatible, identity-mismatched, policy-mismatched, or TTL-expired evidence starts one background verification for that identity. Broker, filesystem, structured processing, audit/activity, binary transfer, and rollback remain available regardless of the Sandbox-only result.
...
7. verifies and journals the target-scoped after checkpoint, and restores only that declared scope after a detected failure. Workspace-relative identifiers use one `/`-separated canonical form in results, checkpoints, and journals on every OS. A successfully recovered failure is reconciled with its terminal audit record immediately; interrupted or unresolved recovery checkpoints remain fail-closed, are reconciled on startup, and are retention-protected.
...
Ordinary file reads do not take a mutation lock. `read_file` is a strict UTF-8 text surface. If decoding fails, it rejects the input without replacement decoding or byte mutation and directs binary-file callers to the byte-exact `artifact_download_begin` path.

`workspace_tree` and `workspace_search` perform bounded recursive traversal inside the Broker. `read_files` returns multiple bounded UTF-8 files while preserving the path, content, byte count, and SHA-256 metadata of `read_file`. These high-level reads reuse `Workspace` path/reparse policy and verified-handle reads, and add request-wide depth, entry, file, byte, and result limits.

`text_file_apply` accepts only one exact replacement whose old text occurs exactly once. `workspace_apply` validates every UTF-8 edit and expected SHA-256 before mutation, acquires the existing canonical target slots and per-target locks in deterministic order, then applies all files through one scoped checkpoint and existing workspace-restore transaction. It records one top-level operation ID. After mutation starts it never retries by invoking generic low-level mutation; verification failure follows the existing `failed_recovered`／`recovery_required` recovery states.

### Structured file processing

`structured_file_inspect` and `structured_file_apply` accept only bounded declarative operations. Parsing and transformation happen outside the workspace-wide mutation lock; the verified read handle is released as soon as the bounded source bytes have been captured. This prevents a failed inspection traceback from retaining a handle that blocks a later transactional replacement. XLSX inspection supports both an active-sheet cell range such as `A1:B8` and a sheet-qualified range such as `検証情報!A1:B8`; read-only worksheets whose package omits the optional dimension metadata are sized from their cells within the existing package and element bounds. XLSX apply may use a distinct `.xlsx` `output_path` to preserve the source while atomically creating or replacing an edited copy. Existing sources require `expected_sha256`, existing outputs separately require `expected_output_sha256`, and commit rechecks the source binding as well as the output's compare-and-swap state. A lexically different `output_path` that resolves back to the source is rejected. Commit applies the artifact through the normal checkpoint, journal, atomic replacement, audit, and recovery path.

### Binary transfer lifecycle
...
`completed` download の manifest と immutable snapshot は admission から外した後も artifact retention の対象として保持し、process restart 後も `open` へ戻さない。保持中は同じ `transfer_id` の有効な chunk retry を byte-exact に返すため、terminal response loss は新しい transfer を要求しない。`artifact_transfer_cancel` は `preparing`／`open` を `cancelled` にし、既に終端状態なら変更せず同じ結果を返す。active transfer は `approval_request_ttl_seconds` を超えると `expired` になり、durable payload identity の破損は `failed` になる。client の offset、base64、未完了 commit など修正可能な入力誤りだけでは terminal failure にしない。全終端状態は admission count から除外するが、data directory quota と retention pruning は引き続き適用する。監査 operation と転送 snapshot の retention は独立しており、親 operation が先に pruning された後の正当な終端 retry／冪等 cancel は、新しい独立 operation として監査してから結果を返す。
...
- Git repository/config override environment variables are removed. Raw system/global Git config and system attributes are disabled in the child. Credential prompting, optional locking, and Git protocol access are disabled. The Broker reconstructs only the trusted scalar `core.autocrlf` semantics required for working-tree status/diff behavior. Project attribute files are replaced by same-length inert comments rather than executed. For an attribute-controlled repository, only source index entries whose ctime、mtime、size still exactly match the source file may have their stat cache rebound to the copied projection identity; stale stat evidence fails closed instead of allowing `status` and metadata-only `diff` to disagree.
...
- Git-specific live marker schema v1 binds pinned Git identity, current Sandbox backend, current generic live-evidence digest, containment-policy generation v7, workspace, configured scratch quota, command-policy generation v5, trusted process-cwd/fixed-`-C`, exact projection ownership trust, sanitized EOL／attribute stat semantics, and required-builtin policy. `verify-git-broker` runs real pinned Git under the same containment, requires worktree recognition and read-only `status` to succeed under strict source-workspace deny, and requires the probe batch to remain bound to one valid sanitized-projection snapshot digest before atomically issuing the marker. User-facing `--show-toplevel` path remapping is not treated as independent projection proof.
...
- Approved Sandbox の child 起動前後では Automatic Git の追加 snapshot を暗黙実行しません。承認済み immutable projection と workspace checkpoint を security／rollback の正本とし、複数の Sandbox 起動を伴う任意の Git telemetry が one-shot execution TTL や command runtime を消費しないようにします。Git 状態を明示取得する `git_info` は独立した Broker 操作として維持します。
...
同期 Broker 操作は `perf_counter_ns()` で測定した全体時間と、開始順の bounded な
処理段階を Audit の `timing_json` に保存し、`audit_get` の `timings` として公開する。
`duration_ms` はこの全体時間の整数ミリ秒で、非同期 worker の既存実行区間とは区別する。
計測値は機械解析用の診断情報であり、安全性判断に使わない。phase ごとの DB commit や
Activity event は追加せず、終了時に一括保存する。既存の監査・journal の永続保証は維持する。
nullable 列追加で旧 DB を移行し、既存行の欠損値を推測で補完しない。
正確な区間、schema、128 段階の上限、入れ子、例外・中断・互換性の規則は
[Audit の処理時間診断](docs/AUDIT_PERFORMANCE.md) に定める。
...
`activity_timeline` is a summary projection only: operation/time/tool/type/status, a short command or target, changed-file and line counts, point-in-time rollback/selective-Undo availability, conflict state, network enforcement, and important risk. It never expands unified diffs, output previews, events, or full path lists. `activity_get(operation_id)` is the bounded detail projection for those artifacts and technical fields. `operation_report(operation_id)` combines the usual status, tool, high-level operation, route, bounded request/result summaries, workspace changes, approval, rollback, and major audit/activity events in one read-only call. The detailed `audit_get` and `activity_get` primitives remain available. The CLI follows the same list/detail split. Reading these views is audited and creates no execution route.
...
### Activity Monitor and Approval UI Live Activity

The Activity Monitor is a read-only low-level observer of `audit.db`. It reports operation lifecycle creation and updates with tool, route, status, approval status, and a bounded redacted summary. Its technical output is not the Approval UI's human activity projection.

Approval UI Live Activity projects only user-meaningful current activity: file and artifact reads, mutations, structured processing, command/device activity, transfer, approval, failure, rejection, cancellation/interruption, Selective Undo, and point-in-time rollback. Successful Selective Undo is `Undone`; successful point-in-time rollback is `Rolled back`. A high-level operation is one top-level Live Activity item; its preflight, locks, per-file work, verification, fallback decision, and automatic recovery remain child events on the same Audit operation. Directly invoked low-level tools remain independent operations. Structured processing uses only the format and target metadata already validated and stored in Audit. Artifact begin/chunk/commit records remain separate Audit operations/events but are correlated into a bounded logical upload or download lifecycle for display. Audit access, Timeline access, `operation_report`, session/capability inspection, polling, and other pure diagnostic or metadata operations are normally suppressed.

The Live Activity projection reads no file body or diff to construct a summary. It allowlists bounded Audit metadata, applies redaction and terminal control/bidirectional/surrogate handling, and never includes raw request payloads, stdout/stderr bodies, file contents, diff bodies, credentials, request hashes, or operation IDs in ordinary success lines. Abnormal states may add only a short safe route or operation-kind hint. At startup it does not replay completed history, but it does surface important pending, approved, queued, running, and committing work, plus a transfer that Audit events show is still in progress. Repeated polling and protocol chunk events do not create duplicate human lifecycle lines.

Live Activity is observational only. Its category, summary, cache, and displayed text are never inputs to approval, policy, route selection, checkpoint validation, transaction, recovery, rollback, Undo, or any other security decision. Audit remains the complete technical record; Timeline remains the bounded historical list/detail interface.
...
The MCP client selects the target operation. The local Approval UI is the execution boundary for reviewing the supplied target and preview; it does not select the target and cannot be bypassed by the client. Live Activity describes the pending/running/result lifecycle but does not grant approval.

Before either mutation writes the workspace, every referenced blob is re-hashed, the current state is checked against the approval preview, and required target bytes are staged. A durable transaction journal records preflight, staging, applying, recovery, and completion. Apply failures automatically attempt restoration of the transaction-start state. A recovered failure is `failed_recovered`; after its audit record reaches a terminal state it does not block later mutations. A failed or unverified recovery is `recovery_required` and continues to block mutation, rollback, and Undo until explicit reconciliation succeeds. Startup reconciliation surfaces journals left incomplete by process interruption. `complete` is written only after the final workspace hashes match the intended target. This is failure-atomic best effort over multiple files, not a claim of an OS filesystem transaction.
...
Policy input acceptance is not equivalent to a verified boundary. Live evidence schema v6 records `filesystem_read`, `filesystem_write`, `protected_information_read`, `internet`, `lan`, `loopback`, `descendant_containment`, `termination`, and `resource_bound` separately as `verified`, `failed`, or `unverified`, and additionally requires `brokered_process_creation_denied=true`. `failed` requires an executed probe to observe a boundary escape; launch failure, timeout, listener or probe setup failure, and other diagnostic inability are `unverified`. Descendant containment individually measures source-write, outside-user read, protected-information read, control-plane read/write, Internet, LAN, and loopback for child and grandchild. Resource verification exceeds both Job limits and proves violation reporting, safe termination, zero remaining descendants, and WLMCP exit-state collection. Schema v1-v5, a missing or unknown `verification_status`, missing mandatory fields, a changed `isolation_context_digest`, a missing/false brokered-process denial, and partially verified mandatory property sets are rejected without inference or migration. Schema v6 binds the exact imported WFP Guard module canonical paths, content SHA-256 values, Windows stable file identities, sizes, Guard version, policy generation, Windows product/build/UBR/native architecture, Sandbox account identity, and stable WFP read-back identity. The isolation context additionally binds the installed launcher/helper identities, physical roots, protected names and directories, dependency-readable paths, policy generations, scratch quota, and process/memory/TTL limits.

A stale marker makes the Sandbox operation route unavailable immediately. Individual Sandbox and Automatic Git requests remain read-back-only and never initiate UAC, repair, or fallback. The LocalMCP startup lifecycle alone may start the same hardened verifier in the background; manual `verify-codex-sandbox` remains a diagnostics/forced-reverification entrypoint to that same core. Both paths close the Sandbox route while verification is running. Only the complete verification unit may recreate exact missing static non-persistent WFP objects once at its beginning; every later probe performs a fresh unelevated read-back, and any intervening loss or mismatch stops without another elevation. Existing security-relevant mismatches or conflicting objects are never silently repaired.

通常ユーザーによる WFP 読み取りの初回設定には、明示的な管理者コマンド `--maintenance-prepare-read-access` を使用します。完全な固定 policy 検証後に、App Isolation と Guard のサブレイヤー、Guard の IPv4／IPv6 フィルターの計4個だけへ、実行者 SID の `FWPM_ACTRL_READ`（`0x80`）を追加します。既存 DACL の ACE・拒否・所有者を保持し、engine／container、変更・削除権限は対象外です。既存の昇格 Guard による正確な欠損の再構築でも、通常権限の後続検証に必要な読み取りを設定します。読み取り失敗からの自動昇格は追加せず、App Isolation のアクセス権が失われた場合は明示的な設定を再実行します。手順は `docs/WFP_READ_ACCESS.md` を参照します。

Automatic verification is serialized with a named process-shared OS file lock under `data_dir`, and marker validity is rechecked after lock acquisition. Sandbox execution holds the same lock through child creation and rechecks the marker after preflight, so verification start, marker replacement, or TTL expiry cannot race into a child launch. OS lock release permits recovery after process crash or power loss. A separate identity-bound attempt record preserves `last_attempt_at`, terminal `failed`/`unverified` state, and a bounded failure reason. Automatic retry observes a configurable cooldown for the same identity, while backend/helper, physical-root, isolation-context, policy-generation, current Sandbox-account, or WFP-binding changes produce a new identity and may retry immediately. The manual verifier bypasses cooldown. Session status keeps dependency/startup `available`, `live_verification_status`, timestamps/reasons, aggregate `windows_live_verified`, and policy-gated `execution_route_available` separate.
...
`run-localmcp.bat` は LocalMCP と同じ寿命の活動監視プロセスを起動します。監視プロセスは `<data_dir>\audit.db` を SQLite の読み取り専用モードで定期確認し、起動後に作成または状態変更された操作だけを、操作 ID、ツール、実行経路、状態、承認状態、伏せ字化して長さを制限したコマンド／パス／対象の要約の一行として起動ターミナルと `<data_dir>\logs\localmcp-activity.log` へ同時出力します。承認待ちは明示し、承認／拒否は別のローカル承認プロセスが所有します。`approval_ui_autostart=true`では、選択済みserver runtimeの`run-server.ps1`と同じdirectoryにある`run-approvals.ps1`を、同じconfigを指定した通常ユーザーの可視なWindows PowerShell 5.1別ウィンドウとして起動します。config絶対path由来の名前付きmutexにより、手動起動を含め同じconfigの承認UIは一つだけです。runnerが自動起動したUIだけを追跡し、終了時は通常のwindow closeを要求します。応答しないUIの親PowerShellだけを強制終了してPython UIを孤立させず、警告して手動終了を求めます。自動起動失敗は承認を省略せず、server起動を継続して手動UIを利用可能にします。Approved Host stateでは検証済みinstalled runtimeのsiblingだけを使い、repositoryへfallbackしません。監視ログは5 MiB、10世代でファイルを切り替え、生の要求／結果JSON、ファイル内容、差分、標準出力／標準エラーのプレビュー、Tunnel clientの生出力、認証情報は複製しません。監視の起動失敗はserver routeのsecurity gateではないため警告してserver起動を継続しますが、Tunnel／runtime／credentialの失敗は従来どおりfail closedです。
...
- pure local reads, including `workspace_tree`, `workspace_search`, `read_files`, `artifact_download`, `operation_report`, and `execute_readonly`: read-only, non-destructive, closed-world. Git requests either execute through the verified metadata-only Automatic Git Broker or fail closed before Git child creation;
- `adb_read`: read-only, non-destructive, closed-world;
- `write_file`, `text_file_apply`, `workspace_apply`, `artifact_upload`, and `execute_workspace_write`: non-read-only, destructive, closed-world;
