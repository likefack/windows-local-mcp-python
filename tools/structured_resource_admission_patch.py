...
SOURCE = Path("src/windows_local_mcp/structured_files.py")
TESTS_PATH = Path("tests/test_structured_resource_admission.py")
...
'''
heading = "# 検証記録\n\n"
if not verification_text.startswith(heading):
    raise RuntimeError("unexpected VERIFICATION.md heading")
VERIFICATION.write_text(
    note + verification_text[len(heading):],
    encoding="utf-8",
    newline="\n",
)
