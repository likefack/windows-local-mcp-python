...
if os.name == "nt":
    ...
    _HANDLER_FUNCTION = ctypes.WINFUNCTYPE(wintypes.DWORD, wintypes.DWORD, wintypes.DWORD, ctypes.c_void_p, ctypes.c_void_p)
    _SERVICE_MAIN_FUNCTION = ctypes.WINFUNCTYPE(None, wintypes.DWORD, ctypes.POINTER(wintypes.LPWSTR))

    class _SERVICE_TABLE_ENTRY(ctypes.Structure):
        _fields_ = [("lpServiceName", wintypes.LPWSTR), ("lpServiceProc", _SERVICE_MAIN_FUNCTION)]
    ...
    _advapi32.ConvertStringSecurityDescriptorToSecurityDescriptorW.argtypes = [wintypes.LPCWSTR, wintypes.DWORD, ctypes.POINTER(ctypes.c_void_p), ctypes.POINTER(wintypes.DWORD)]
    _advapi32.ConvertStringSecurityDescriptorToSecurityDescriptorW.restype = wintypes.BOOL
    _advapi32.RegisterServiceCtrlHandlerExW.argtypes = [wintypes.LPCWSTR, _HANDLER_FUNCTION, ctypes.c_void_p]
    _advapi32.RegisterServiceCtrlHandlerExW.restype = wintypes.HANDLE
    _advapi32.SetServiceStatus.argtypes = [wintypes.HANDLE, ctypes.POINTER(_SERVICE_STATUS)]
...
def _process_token_details(pid: int) -> tuple[str, bool]:
    ...
    try:
        ...
        if not _advapi32.GetTokenInformation(token, _TOKEN_USER, user_buffer, len(user_buffer), ctypes.byref(needed)):
            raise _winerror("GetTokenInformation(TokenUser)")
        token_user = ctypes.cast(user_buffer, ctypes.POINTER(_TOKEN_USER_VALUE)).contents
        sid_text = wintypes.LPWSTR()
        if not _advapi32.ConvertSidToStringSidW(token_user.Sid, ctypes.byref(sid_text)):
        ...
        if not _advapi32.GetTokenInformation(token, _TOKEN_ELEVATION, ctypes.byref(elevation), ctypes.sizeof(elevation), ctypes.byref(returned)):
...
class ApprovedHostAuthorityServer:
    def __init__(self, *, runtime_sid: str, state_root: Path) -> None:
        ...
        if self.store.read_active() is not None:
            self.store.mark_recovery_required("authority service restarted while an operation was active")
    ...
    def serve(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                ...
                if not _kernel32.GetNamedPipeClientProcessId(pipe, ctypes.byref(client_pid)):
                ...
                except Exception as error:  # noqa: BLE001 - RPC errors are returned fail closed
                    response = {
    ...
    def _read_message(self, pipe: wintypes.HANDLE) -> dict[str, Any]:
        ...
        while True:
            ...
            ok = _kernel32.ReadFile(pipe, buffer, len(buffer), ctypes.byref(read), None)
            if read.value:
    ...
    def _write_message(self, pipe: wintypes.HANDLE, response: Mapping[str, Any]) -> None:
        ...
        if not _kernel32.WriteFile(pipe, buffer, len(data), ctypes.byref(written), None):
    ...
    def handle_request(self, client_pid: int, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if client_sid.casefold() != self.runtime_sid.casefold():
            raise PermissionError("Approved Host authority client SID is not the configured runtime user")
        if elevated:
            raise PermissionError("Approved Host authority requires a non-elevated requester token")
        ...
        if action == "cancel":
            return self._cancel(client_pid, request)
        raise ValueError("unknown Approved Host authority action")

    def _launch(self, client_pid: int, request: Mapping[str, Any]) -> dict[str, Any]:
        requester_pid = int(request.get("requester_pid") or 0)
        if requester_pid != client_pid:
            raise PermissionError("Approved Host authority requester PID does not match pipe peer")
        ...
        if abs(actual_create_time - requester_create_time) > 0.01:
            raise PermissionError("Approved Host authority requester process identity changed")
        ...
        if not isinstance(environment_value, dict):
            raise TypeError("Approved Host authority worker environment must be an object")
        worker_environment = {str(key): str(value) for key, value in environment_value.items()}
        if worker_environment.get("WINDOWS_LOCAL_MCP_JOB_NONCE") != process_nonce:
            raise PermissionError("Approved Host authority worker nonce binding changed")
        if sum(len(key) + len(value) for key, value in worker_environment.items()) > 256 * 1024:
            raise ValueError("Approved Host authority worker environment exceeds its bound")
        ...
        proof_path = authority_completion_path(self.store.root, operation_id, authority_nonce)
        self.store.arm(
        ...
        try:
            ...
            self.store.mark_recovery_required(f"SYSTEM worker launch failed: {type(error).__name__}: {error}")
            raise
    ...
    def _watch_worker(self, operation_id: str, process: subprocess.Popen[Any], proof_path: Path) -> None:
        ...
        except Exception as error:  # noqa: BLE001 - completion uncertainty stays latched
            self.store.mark_recovery_required(
                f"authority completion verification failed: {type(error).__name__}: {error}",
                worker_exit_code=exit_code,
            )
        finally:
            with self._workers_lock:
    ...
    def _cancel(self, client_pid: int, request: Mapping[str, Any]) -> dict[str, Any]:
        del client_pid
        operation_id = str(request.get("operation_id") or "")
        active = self.store.read_active()
        if active is None or active.get("operation_id") != operation_id:
            raise RuntimeError("Approved Host authority has no matching active operation")
        with self._workers_lock:
            process = self._workers.get(operation_id)
        if process is None:
            self.store.mark_recovery_required("cancellation requested after worker identity was lost")
            raise ApprovedHostRecoveryRequired(
        ...
        if not handle:
            self.store.mark_recovery_required("cancellation could not open the SYSTEM worker")
        ...
        try:
            ...
            _kernel32.CloseHandle(handle)
        self.store.mark_recovery_required(
            "operator cancellation terminated the SYSTEM worker; postflight completion was not proven"
        )
        return {
            "protocol_version": APPROVED_HOST_AUTHORITY_PROTOCOL_VERSION,
            "ok": True,
...
class _WindowsServiceHost:
    def __init__(self, server_factory: Callable[[], ApprovedHostAuthorityServer]) -> None:
    ...
    def _set_status(self, state: int, *, controls: int = 0, win32_exit: int = 0, wait_hint: int = 0) -> None:
        ...
        if not _advapi32.SetServiceStatus(self.status_handle, ctypes.byref(status)):
    ...
    def _service_main(self, _argc: int, _argv: ctypes.POINTER(wintypes.LPWSTR)) -> None:
        ...
        try:
            ...
            thread = threading.Thread(target=self.server.serve, name="wlmcp-approved-host-pipe", daemon=True)
            thread.start()
            self._set_status(
                _SERVICE_RUNNING,
    ...
    def _handler(self, control: int, _event_type: int, _event_data: ctypes.c_void_p, _context: ctypes.c_void_p) -> int:
...
def main() -> None:
    ...
    parser.add_argument("--state-root", type=Path, default=default_authority_state_root())
    parser.add_argument("--console", action="store_true")
    args = parser.parse_args()
    factory = lambda: ApprovedHostAuthorityServer(runtime_sid=args.runtime_sid, state_root=args.state_root)
    if args.console:
        server = factory()
        try:
            server.serve()
