...
9. protected information への read access は Broker policy と OS boundary の両方で deny し、Git config/filter 等を経由した stdout/stderr への持ち出しを許可しません。projection 内の Git object database に protected file の historical blob が存在し得ることも前提にし、raw blob/tree object ID を content-producing grammar へ渡せることを security boundary とみなしません。
...
Git object database は repository semantics のため projection に含まれますが、model-facing content access は object type と Broker path policy の両方で制限します。Automatic `show` と user-supplied `diff` revision は `^{commit}` へ binding し、range は両 endpoint を commit-bound にします。これにより `ls-files --stage` 等で blob SHA が既知になっても `git show --no-patch <blob-sha>` のような raw object content read を許可しません。`diff --check` は content-bearing output として扱い、Broker が検証した regular-file pathspec がある場合だけ automatic grammar に残します。
